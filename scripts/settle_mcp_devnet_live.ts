...
export async function runMcpLiveSettlement() {
  console.log(`============================================================`);
  console.log(`   Executing Live Devnet Relayed V3 Settlement for MCP      `);
  console.log(`============================================================`);

  const provider = new ApiNetworkProvider(DEVNET_API, { clientName: "mcp-live-settler" });
  ...
  // 4. MCP Tool Execution: on-chain settlement fee (10,000 micro-USDC = $0.01)
  const amountMicroUsdc = 10000;
  ...
  console.log(`\n🚀 Relayed V3 MCP Settlement Broadcasted!`);
  ...
  console.log(`\n✅ MCP Settlement SUCCESSFUL on MultiversX Devnet! Status: ${finishedTx.status.status}`);
...
if (process.argv[1]?.includes("settle_mcp_devnet_live")) {
  runMcpLiveSettlement().catch((err) => {
